...
input_IZ_table <- "IZ2011_NHS24_Calls_2020_2019_March_to_May"
...
data <- read_csv(glue("{filepath_outputs}/{input_IZ_table}.csv")) %>% 
  mutate(NHS24Calls_rate_2019 = round_half_up(Calls_rate_per_1000_pop_2019,1), 
         NHS24Calls_rate_2020 = round_half_up(Calls_rate_per_1000_pop_2020,1),
         perc_diff = round_half_up(perc.diff.calls, 1)) %>% 
  select(InterZone = intzone2011,
         Name = intzone2011_name,
         HB, HBName,
         NHS24Calls_rate_2019, NHS24Calls_rate_2020,
         perc_diff) 

int_dat <- data %>% 
  arrange(desc(NHS24Calls_rate_2020))
...
# ramp low dark red though yellow to
#high light blue to dark purple
pal_diff <- colorNumeric(palette = colorRampPalette(c("#c01618","#FC4E2A",
                                                      "#FEB24C","#FFEDA0", 
                                                      "#ffffff",
                                                      "#e4f0f6","#a9cfe2", "#6eafcf",
                                                      "#327496","#23526a",
                                                      "#6a2352","#5b1e47","#4d193b",
                                                      "#3e1430","#200b19"
                                                      ))(length(int_shp$perc_diff)),
...
map.change <- leaflet() %>%
  ...
  addPolygons(data = int_shp, 
              ...
              popup = paste0("Intermediate Zone: <b>", int_shp$Name.x,
                             ...
                             "</b><br>NHS24 Completed Calls per 1000 people 2019: <b>", int_shp$NHS24Calls_rate_2019,
                             "</b><br>NHS24 Completed Calls per 1000 people 2020: <b>", int_shp$NHS24Calls_rate_2020,
                             "</b><br>Change in NHS24 Completed Calls from 2019 to 2020: <b>", paste0(int_shp$perc_diff,"%"), "</b>")) %>% 
...
table_data <- read_csv(glue("{filepath_outputs}/{input_IZ_table}.csv")) %>% 
  mutate(NHS24Calls_rate_per_1000_pop_2019 = round_half_up(Calls_rate_per_1000_pop_2019,1), 
         NHS24Calls_rate_per_1000_pop_2020 = round_half_up(Calls_rate_per_1000_pop_2020,1),
         perc.diff.calls = round_half_up(perc.diff.calls, 1))%>% 
  select(IntZone2011 = intzone2011, 
         IntZone2011_Name = intzone2011_name, 
         Health_Board_Code = HB,
         Health_Board_Name = HBName,
         NHS24_Completed_Calls_2019 = completed_calls_nhs24_2019, 
         NHS24_Completed_Calls_2020 = completed_calls_nhs24_2020, 
         Total_Population_Estimated_2018 = Total_Pop_IZ_2018, 
         NHS24_Completed_Calls_Rate_per_1000_people_2019
         = Calls_rate_per_1000_pop_2019,
         NHS24_Completed_Calls_Rate_per_1000_people_2020 
         = Calls_rate_per_1000_pop_2020,
         Percentage_Difference_NHS24_Completed_Calls
         = perc.diff.calls)
