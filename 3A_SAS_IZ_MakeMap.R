...
input_IZ_table <- "IZ2011_SAS_Attendances_2020_2019_March_to_May"
...
data <- read_csv(glue("{filepath_outputs}/{input_IZ_table}.csv")) %>% 
  mutate(SASAttend_rate_2019 = round_half_up(Atts_rate_per_1000_pop_2019,1), 
         SASAttend_rate_2020 = round_half_up(Atts_rate_per_1000_pop_2020,1),
         perc_diff = round_half_up(perc.diff.atts, 1)) %>% 
  select(InterZone = intzone2011,
         Name = intzone2011_name,
         HB, HBName,
         SASAttend_rate_2019, SASAttend_rate_2020,
         perc_diff) 

int_dat <- data %>% 
  arrange(desc(SASAttend_rate_2020))
...
pal_diff <- colorNumeric(palette = colorRampPalette(c("#FC4E2A","#FD8D3C","#FEB24C", 
                                                      "#FED976", "#FFEDA0","#FFFFCC", 
                                                      "#ffffff",
                                                      "#e4f0f6","#c7e0ec", "#a9cfe2", 
                                                      "#8cbfd9","#6eafcf","#519ec5",
                                                      "#3c8bb3","#327496","#2d6987",
                                                      "#23526a"
...
map.change <- leaflet() %>%
  ...
  addPolygons(data = int_shp, 
              ...
              popup = paste0("Intermediate Zone: <b>", int_shp$Name.x,
                             ...
                             "</b><br>SAS Attendances per 1000 people 2019: <b>", int_shp$SASAttend_rate_2019,
                             "</b><br>SAS Attendances per 1000 people 2020: <b>", int_shp$SASAttend_rate_2020,
                             "</b><br>Change in SAS Attendances from 2019 to 2020: <b>", paste0(int_shp$perc_diff,"%"), "</b>")) %>% 
...
table_data <- read_csv(glue("{filepath_outputs}/{input_IZ_table}.csv")) %>% 
  mutate(SASAttend_rate_per_1000_pop_2019 = round_half_up(Atts_rate_per_1000_pop_2019,1), 
         SASAttend_rate_per_1000_pop_2020 = round_half_up(Atts_rate_per_1000_pop_2020,1),
         perc.diff.atts = round_half_up(perc.diff.atts, 1))%>% 
  select(IntZone2011 = intzone2011, 
         IntZone2011_Name = intzone2011_name, 
         Health_Board_Code = HB,
         Health_Board_Name = HBName,
         SAS_Attendances_2019 = sas_attendances_2019, 
         SAS_Attendances_2020 = sas_attendances_2020, 
         Total_Population_Estimated_2018 = Total_Pop_IZ_2018, 
         SAS_Attendances_Rate_per_1000_people_2019
         = Atts_rate_per_1000_pop_2019,
         SAS_Attendances_Rate_per_1000_people_2020 
         = Atts_rate_per_1000_pop_2020,
         Percentage_Difference_SAS_Attendances
         = perc.diff.atts)
