...
#####  in Rstudio 1.1.456 and RStudio Server
...
input_RAPID <- "historical_admissions"
...
# set as from 1 March 2019 to 31st May 2019
key_date_start <- ymd(20190301)
key_date_end <- ymd(20190531)

#set date range to append to any output and check files
date_range <- "20190301_to_20190531"


#### set up definition lists 
# Set codes used for an Emergency Admission
emergency_admission_type <- c(20,21,22,30,31,32,33,34,35,36,38,39)

# set excluded specialty group
...
RAPID_admissions <- read_csv(glue("{filepath_RAPID_In}/{input_RAPID}.csv"))


# check column names
colnames(RAPID_admissions)

#rename and filter required variables
RAPID_admissions_keyvar <- RAPID_admissions %>% 
...
write.csv(check_emergency_admit_coding_all,
          (glue("{filepath_checks}/check_emergency_admit_coding_all_{date_range}.csv")))
...
write.csv(check_admission_group_all,
          (glue("{filepath_checks}/check_admission_group_all_{date_range}.csv")))
...
write.csv(check_specialty_coding_all_S,
          (glue("{filepath_checks}/check_specialty_coding_all_S_{date_range}.csv")))

### Assuming G1 to 4 codes currently coded as other - then only need to exlcude SG 0
...
write.csv(check_distinct_hospitals_all_S,
          (glue("{filepath_checks}/check_distinct_hospitals_all_S_{date_range}.csv")))
...
write.csv(check_admission_group_filtered,
          (glue("{filepath_checks}/check_admission_group_filtered_{date_range}.csv")))
...
write.csv(check_admission_with_IG,
          (glue("{filepath_checks}/check_admission_withIG_{date_range}.csv")))

# check what data looks like aggregated to health board level

#cannot do this with health board of residence in file 
...
#          (glue("{filepath_checks}/check_HB_Residence_NoIG_{date_range}.csv")))
  


check_HB_PCfilesCodes_EmergencyAdmissions <- RAPID_admissions_filtered_wPC %>% 
  group_by(HB2018, HB2019, HB2019Name) %>% 
  arrange(HB2018) %>% 
  summarise_at(vars(Number_Observations,Number_of_Stays),sum) %>% 
...
View(check_HB_PCfilesCodes_EmergencyAdmissions)

write.csv(check_HB_PCfilesCodes_EmergencyAdmissions,
          (glue("{filepath_checks}/check_HB_PCfilesCodes_EmergencyAdmissions_{date_range}.csv")))
